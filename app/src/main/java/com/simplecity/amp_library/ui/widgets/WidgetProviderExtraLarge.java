...
public class WidgetProviderExtraLarge extends BaseWidgetProvider {
    ...
    public void update(MusicService service, SharedPreferences sharedPreferences, int[] appWidgetIds, boolean updateArtwork) {
        ...
        for (int appWidgetId : appWidgetIds) {
            updateSingleWidget(service, sharedPreferences, appWidgetId, appWidgetIds, updateArtwork);
        }
    }

    private void updateSingleWidget(MusicService service, SharedPreferences sharedPreferences, int appWidgetId, int[] appWidgetIds, boolean updateArtwork) {
        boolean showAlbumArt = sharedPreferences.getBoolean(ARG_WIDGET_SHOW_ARTWORK + appWidgetId, true);
        ...
        setWidgetTextViews(views, errorState, titleName, artistName, albumName);
    ...
    private void setWidgetTextViews(RemoteViews views, CharSequence errorState, CharSequence titleName, CharSequence artistName, CharSequence albumName) {
        if (errorState != null) {
            views.setViewVisibility(R.id.text1, View.GONE);
